...
const apiPre = config.get('backend.apiPrefix');
...
describe("Authentification Tests ", () => {
  ...
  it("AB_03b POST - Login user - Check Information Object - 200", (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        chai
          .expect(res.body.information.user.username)
          .eql(validCredentials.username);
        token = res.body.token;
        done();
      });
  ...
  it("AB_08 POST - Login user again - 200", (done) => {
    chai
      .request(server)
      .post(apiPre + "/login")
      .send(validCredentials)
  ...
  it("AB_09 DELETE first user - 204", (done) => {
    chai
      .request(server)
  ...
  it("AB_10 Do a get call with token for which no user exists anymore - 200", (done) => {
    chai
      .request(server)
      .get(apiPre + "/whoIAm")
      .set("Authorization", "Bearer " + token)
  ...
  it("AB_11 Fail to Login - invalid credential object - 401", (done) => {
  ...
  it("AB_12 Fail to Login - no credential object - 401", (done) => {
    chai
      .request(server)
      .post(apiPre + "/login")
      .end((err, res) => {
  ...
  it("AB_13 Fail to Login - credential object with too many properties - 401", (done) => {
    let invalidCredentials = {
      username: "newman-r@user.com",
      password: "1234",
      noneedprop: "1234",
